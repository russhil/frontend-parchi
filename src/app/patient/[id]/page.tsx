    ...
    return (
      <div className="p-4 md:p-6">
        <div className="space-y-4">
          {[1, 2].map((i) => (
            <div key={i} className="bg-surface rounded-2xl border border-border-light shadow-sm p-5 animate-pulse">
  ...
  return (
    <div className="p-4 md:p-6">
      ...
      <div className="mb-5 flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
        <h1 className="text-base md:text-lg font-bold text-text-primary">Patient Overview</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
        ...
        <div className="lg:col-span-8 space-y-5">
          ...
          <div className="bg-surface rounded-2xl border border-border-light shadow-sm overflow-hidden">
            <div className="px-4 md:px-5 py-4 border-b border-border-light flex items-center gap-2">
                ...
                {appointments.slice(0, 5).map((appt) => {
                  ...
                  return (
                    <Link
                      ...
                      className="block px-4 md:px-5 py-3.5 hover:bg-gray-50 transition-colors group"
                    >
                      {/* Desktop Layout */}
                      ...
                      <div className="sm:hidden space-y-2">
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <div className="flex flex-col items-end gap-1">
                            <span className={`px-2 py-0.5 rounded-full text-[10px] font-medium whitespace-nowrap ${badge.bg} ${badge.text}`}>
                              {badge.label}
                            </span>
                            <span className="material-symbols-outlined text-gray-300 text-[18px] group-hover:text-primary transition-colors">
                              chevron_right
            ...
            {appointments.length > 5 && (
              <div className="bg-gray-50 px-5 py-3 border-t border-border-light text-center">
                <span className="text-xs text-text-secondary">Showing recent 5 appointments</span>
              </div>
            )}
          </div>
          ...
          <div className="bg-surface rounded-2xl border border-border-light shadow-sm overflow-hidden">
            <div className="px-4 md:px-5 py-4 border-b border-border-light flex items-center gap-2">
              ...
              <Link href={`/patient/${patientId}/documents`} className="ml-auto text-xs text-primary hover:underline font-medium">
                View All ({data.documents?.length || 0})
              </Link>
            </div>
            {data.documents && data.documents.length > 0 ? (
              <div className="divide-y divide-border-light">
                {data.documents.slice(0, 5).map(doc => (
                  <div key={doc.id} className="px-4 md:px-5 py-3.5 hover:bg-gray-50 transition-colors flex items-center justify-between group">
                    <div className="flex items-center gap-3 min-w-0">
                      <div className={`w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 bg-gray-100 text-gray-600`}>
